    ...
    try:
        ...
        for model_name in ['gemini-2.0-flash', 'gemini-1.5-flash-latest', 'gemini-flash-latest']:
            try:
                ...
                prompt = f"""あなたは「現役の化粧品開発者」であり、かつ「仕事帰りのカフェでの資格勉強に頑張る社会人受験生」という２０代の二つの顔を持つ女性です。以下の商品について紹介文を作成してください。\n【商品名】: {item_name}\n【商品説明】: {item_caption}\n【制約】: 薬機法配慮、親しみやすい、#PR入り、ハッシュタグを3～5個つけてください、200文字程度。"""
...
with tab1:
    ...
    keywords = ["韓国コスメ", "スキンケア", "便利 文房具", "デスク周り 癒し", "アルコール", "人気コスメ"]
    random.seed(date.today().toordinal())
    daily_keyword = random.choice(keywords)
    items = search_rakuten(daily_keyword, hits=5)
